...
export function CatalogPage() {
  ...
  useEffect(() => {
    ...
    async function loadAuctions() {
      ...
      try {
        ...
        if (!ignore) {
          setError(err.message || "Error al consultar las subastas.")
          setAuctions([])
        }
      } finally {
        if (!ignore) {
  ...
  }, [pagina, estado, categoriaId, precioMin, precioMax, ordenamiento, refreshKey])
      ...
      {loading ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {Array.from({ length: 6 }).map((_, idx) => (
            <div key={idx} className="flex flex-col gap-3 rounded-xl border border-border p-4 bg-card">
        ...
        <div className="flex flex-col items-center justify-center py-16 px-4 text-center rounded-xl border border-dashed border-border">
          ...
          <h3 className="text-base font-semibold text-foreground">No se encontraron subastas</h3>
